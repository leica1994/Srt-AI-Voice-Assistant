...
# 项目内部导入
from core import args, MANUAL, audio_separator
from core.utils import *
from core.edit_panel import *
...
⚡ 文件已从缓存快速加载，无需重新处理！
    ...
    try:
        ...
        result = audio_separator.separate_video_audio(
            ...
            max_memory_gb=core.config.max_gpu_memory_gb,
            auto_quality=core.config.auto_quality_adjustment
        )
        ...
        if subtitle_ext in ['.ass', '.vtt']:
            ...
            if subtitle_ext == '.ass':
                # ASS 文件处理
                from core.subtitle_processor import format_ass_file, extract_ass_to_srt, get_available_styles
                ...
                # VTT 文件处理
                from core.subtitle_processor import convert_subtitle
...
def export_subtitle_with_new_name(file_list, subtitle_state):
    ...
    try:
        ...
        if original_files:
            ...
            original_basename = core.utils.basename_no_ext(original_file)
...
def export_original_format(original_file, srt_file, base_name, original_ext, output_dir):
    ...
    try:
        if original_ext == '.ass':
            ...
            from core.subtitle_processor import sync_srt_timestamps_to_ass, format_ass_file
...
# single speaker
def generate(*args, interrupt_event: core.utils.Flag, proj="", in_files=[], fps=30, offset=0, max_workers=1):
    t1 = time.time()
    ...
    if core.config.server_mode and len(in_files) > 1:
    ...
    for in_file in in_files:
        ...
        Projet_dict[proj].before_gen_action(*args, config=core.config, notify=False, force=False)
        ...
        with interrupt_event:
            ...
            if len(file_list) == 0:
                ...
                if len(in_files) == 1:
                    ...
                    continue
        # 优化大文件处理：使用流式音频合并避免内存溢出
        try:
            sr_audio = subtitle_list.audio_join(sr=core.config.output_sr)
        except MemoryError:
            gr.Warning("音频文件过大，正在使用优化模式处理...")
            # 如果内存不足，创建一个临时文件路径而不是加载到内存
            import tempfile
            output_file = tempfile.NamedTemporaryFile(suffix=".wav", delete=False)
            output_file.close()

            # 使用 ffmpeg 或其他工具合并音频文件（如果可用）
            try:
                import subprocess
                audio_files = [os.path.join(abs_dir, f) for f in os.listdir(abs_dir) if f.endswith('.wav')]
                audio_files.sort()  # 确保按顺序合并
                ...
                if audio_files:
                    ...
                    sr_audio = output_file.name  # 返回文件路径而不是音频数据
                else:
                    sr_audio = None
            except (subprocess.CalledProcessError, FileNotFoundError):
                # 如果 ffmpeg 不可用，回退到原始方法但分块处理
                gr.Warning("正在使用备用方法处理大文件...")
                sr_audio = subtitle_list.audio_join(sr=core.config.output_sr)
        except Exception as e:
            gr.Warning(f"音频合并失败: {e}")
            sr_audio = None
    t2 = time.time()
    m, s = divmod(t2 - t1, 60)
...
def gen_multispeaker(interrupt_event: core.utils.Flag, *args,
    ...
    for key in tasks.keys():
        ...
        try:
            args, kwargs = Projet_dict[project].arg_filter(*args)
            Projet_dict[project].before_gen_action(*args, config=core.config)
        except Exception as e:
            ok = False
            gr.Warning(str(e))
            continue
    ...
    if remake:
        ...
        sr_audio = subtitles.audio_join(sr=core.config.output_sr)
...
def save(args, proj: str = None, dir: str = None, subtitle: Subtitle = None):
    ...
    if audio is not None:
        if audio[:4] == b'RIFF' and audio[8:12] == b'WAVE':
            ...
            filepath = os.path.join(dir, f"{subtitle.index}.wav")
            if core.config.remove_silence:
                audio, sr = core.librosa_load.load_audio(io.BytesIO(audio))
                audio = remove_silence(audio, sr)
                sf.write(filepath, audio, sr)
            else:
                with open(filepath, 'wb') as file:
                    file.write(audio)
            if core.config.max_accelerate_ratio > 1.0:
                audio, sr = core.librosa_load.load_audio(filepath)
                target_dur = int(subtitle.end_time - subtitle.start_time) * sr
                if target_dur > 0 and (audio.shape[-1] - target_dur) > (0.01 * sr):
                    ratio = min(audio.shape[-1] / target_dur, core.config.max_accelerate_ratio)
...
def start_gsv():
    ...
    if core.config.gsv_fallback:
        ...
        apath = "api_v2.py"
    if not os.path.exists(os.path.join(core.config.gsv_dir, apath)):
        raise FileNotFoundError(os.path.join(core.config.gsv_dir, apath))
    command = f'"{core.config.gsv_pydir}" "{os.path.join(core.config.gsv_dir, apath)}" {core.config.gsv_args}'
    rc_open_window(command=command, dir=core.config.gsv_dir)
    time.sleep(0.1)
...
def start_indextts():
    # 检查是否配置了启动脚本
    if core.config.indextts_script and core.config.indextts_script.strip():
        # 使用启动脚本/命令
        script_command = core.config.indextts_script.strip()

        # 判断是文件路径还是命令
        if script_command.startswith(('powershell', 'cmd', 'python', 'bash', 'sh')) or ' ' in script_command:
            # 这是一个命令，直接执行
            command = f'{script_command} {core.config.indextts_args}'
            work_dir = core.config.indextts_dir if core.config.indextts_dir else os.getcwd()
            ...
            if not script_dir:
                script_dir = core.config.indextts_dir if core.config.indextts_dir else os.getcwd()
        ...
        # 使用原来的启动逻辑
        if core.config.indextts_pydir == "":
            gr.Warning(i18n(
                'Please go to the settings page to specify the corresponding environment path and do not forget to save it!'))
            return i18n(
                'Please go to the settings page to specify the corresponding environment path and do not forget to save it!')
        apath = "api.py"  # Index-TTS API文件
        if not os.path.exists(os.path.join(core.config.indextts_dir, apath)):
            raise FileNotFoundError(os.path.join(core.config.indextts_dir, apath))
        command = f'"{core.config.indextts_pydir}" "{os.path.join(core.config.indextts_dir, apath)}" {core.config.indextts_args}'
        rc_open_window(command=command, dir=core.config.indextts_dir)
        time.sleep(0.1)
...
def remake(*args):
    ...
    if core.config.server_mode and len(s_txt) > 512:
        gr.Warning("too long!")
        return fp, *load_single_line(subtitle_list, idx)
...
def recompose(page: int, subtitle_list: Subtitles):
    ...
    audio = subtitle_list.audio_join(sr=core.config.output_sr)
    gr.Info(i18n("Reassemble successfully!"))
    return audio, "OK", *show_page(page, subtitle_list)


# 流式音频生成器函数
def numpy_to_mp3_bytes(audio_data, sample_rate):
    """将 numpy 音频数据转换为 MP3 字节流"""
    import io
    import soundfile as sf
...
def streaming_generate_preprocess(interrupt_event, *args, project=None):
    ...
    # 使用生成器进行流式音频生成
    yield from streaming_generate(*args, interrupt_event=interrupt_event, **kwargs)


def streaming_generate(*args, interrupt_event: core.utils.Flag, proj="", in_files=[], fps=30, offset=0, max_workers=1):
    """流式音频生成函数 - 真正的流式实现"""
    import time
    import tempfile
    import soundfile as sf
    ...
    for in_file in in_files:
        ...
        processed_count = 0
        accumulated_audio = []
        sample_rate = core.config.output_sr
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in concurrent.futures.as_completed(futures):
                    ...
                    if item:
                        ...
                        # 每处理一定数量的片段就输出一次中间音频
                        if processed_count % max(1, total_subtitles // 10) == 0:
                            try:
                                # 生成当前进度的部分音频
                                partial_audio = subtitle_list.audio_join(sr=sample_rate)
                                if partial_audio is not None:
                                    # 转换为字节流
                                    audio_bytes = numpy_to_mp3_bytes(partial_audio[1], partial_audio[0])
                                    yield audio_bytes
                                    time.sleep(0.1)  # 短暂暂停
                            except Exception as e:
                                print(f"流式音频生成错误: {e}")
        ...
        # 最终完整音频
        try:
            sr_audio = subtitle_list.audio_join(sr=sample_rate)
            if sr_audio is not None:
                # 转换为字节流
                audio_bytes = numpy_to_mp3_bytes(sr_audio[1], sr_audio[0])
                yield audio_bytes
        except Exception as e:
            print(f"最终音频生成错误: {e}")
    ...
    try:
        ...
        if result and len(result) > 0 and result[0] is not None:
            ...
            if audio is not None and len(audio) == 2:
                ...
                # 转换为字节流
                audio_bytes = numpy_to_mp3_bytes(audio_data, sample_rate)
                gr.Info(i18n("Reassemble successfully!"))
                yield audio_bytes

    except Exception as e:
        gr.Warning(f"重组音频失败: {e}")
        # 输出静音以避免界面错误
        try:
            sample_rate = 22050
            duration = 1.0
            silence = np.zeros(int(sample_rate * duration))
            audio_bytes = numpy_to_mp3_bytes(silence, sample_rate)
            yield audio_bytes
        except:
            pass


def streaming_gen_multispeaker(interrupt_event: core.utils.Flag, *args, remake=False):
    """流式多说话人生成函数"""
    import time
    import tempfile
    import soundfile as sf
    ...
    if result and len(result) > 0 and result[-1] is not None:
        ...
        # 如果没有音频返回，尝试重组现有音频
        try:
            audio = subtitles.audio_join(sr=core.config.output_sr)
            if audio is not None:
                # 转换为字节流
                audio_bytes = numpy_to_mp3_bytes(audio[1], audio[0])
                yield audio_bytes
        except Exception as e:
...
if __name__ == "__main__":
    ...
    with gr.Blocks(title="Srt-AI-Voice-Assistant-WebUI", theme=core.config.theme, analytics_enabled=False) as app:
        STATE = gr.State(value=Subtitles())
        INTERRUPT_EVENT = gr.State(value=core.utils.Flag())
        ...
        with gr.Tabs():
            with gr.TabItem(i18n('Subtitle Dubbing')):
                ...
                with gr.Accordion(
                    ...
                    with gr.Column():
                        ...
                        for x in range(core.config.num_edit_rows):
                        ...
                        with gr.Row(equal_height=True):
                            ...
                            all_selection_btn.click(None, inputs=[], outputs=edit_check_list,
                                                    js=f"() => Array({core.config.num_edit_rows}).fill(true)")
                            ...
                            clear_selection_btn.click(None, inputs=[], outputs=edit_check_list,
                                                      js=f"() => Array({core.config.num_edit_rows}).fill(false)")
            ...
            with gr.TabItem(i18n('Extended Contents')):
                available = False
                from core.extern_extensions.wav2srt_webui import WAV2SRT

                WAV2SRT = WAV2SRT(config=core.config)
            ...
            with gr.TabItem(i18n('Settings')):
                with gr.Row():
                    with gr.Column():
                        SETTINGS = core.settings.Settings_UI(componments=componments)
                        SETTINGS.getUI()
                    with gr.Column():
    ...
    app.queue(default_concurrency_limit=core.config.concurrency_count,
              max_size=2 * core.config.concurrency_count).launch(
        share=args.share,
        server_port=server_port if server_port > 0 else None,
        inbrowser=True,
        server_name='0.0.0.0' if core.config.LAN_access or args.LAN_access else '127.0.0.1',
        show_api=not core.config.server_mode,
    )
